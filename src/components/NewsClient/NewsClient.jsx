            ...
            if (dataFilter && dataFilter.length > 0)
              ...
              actual.push(
                ...
                <div className="bg-white shadow-md rounded-lg overflow-hidden">
                  <img src={dataFilter[index].data().frontPage} alt={`img-${dataFilter[index].data().title}`} className="w-full"/>
                    <div className="p-6">
                      <h3 className="text-xl font-bold mb-2">{dataFilter[index].data().title}</h3>
                        <p className="text-gray-700">{dataFilter[index].data().description}</p>
                          <a href={dataFilter[index].data().URL} className="bg-primary block text-center hover:bg-green-900 text-gray-900 font-bold py-2 px-6 rounded-full uppercase tracking-wide mt-4">Leer más</a>
                  </div>
                  </div>
                </div>
